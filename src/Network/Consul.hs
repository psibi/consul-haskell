...
-- QUESTION: rename `man` to `mManager` or `maybeMan` or `maybeManager` or `mm` or `mman` or `mmanager`?
...
initializeConsulClient hostname port man = do
  ...
  return $ ConsulClient manager hostname port False
                                         --   ^--- does this mean the client implemented here
...
getKey _client@ConsulClient{..} key index consistency dc = do
  -- TODO: can I reduce / simplify this further?
...
runService client request action dc = do
  --r <- registerService client request ccPort dc
  ...
  where
    ttlFunc y@(Ttl x) = do
      ...
      let checkId = T.concat["service:",maybe (rsName request) id (rsId request)]
          --portNumber = (ccPort client)
      -- !QUESTION: do we use the checkId for the hostname?
...
passHealthCheck client checkId dc = do
  ...
  initReq <- createRequest hostname
                           ...
                           False
                           dc
  -- TODO: see warning: `Defined but not used: ‘response’`
